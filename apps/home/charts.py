...
class Chart:
    ...
    def from_lists(self, values, labels, stacks):
        ...
        # type stacks checking
        if stacks is not None:
            if len(stacks) == 1:
                if stacks[0] == 'Income':
                    palette = [self.palette[0]] * len(values[0])
                elif stacks[0] == 'Outcome':
                    palette = [self.palette[1]] * len(values[0])
        ...
        if len(values) == 1:
            self.datasets[0]['backgroundColor'] = palette
